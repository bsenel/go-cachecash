...
var (
	cacheDBTypes = map[string]string{`PublicKey`: `BYTEA`, `Version`: `VARCHAR`, `FreeMemory`: `BIGINT`, `TotalMemory`: `BIGINT`, `FreeDisk`: `BIGINT`, `TotalDisk`: `BIGINT`, `StartupTime`: `DATETIME`, `ExternalIP`: `VARCHAR`, `Port`: `INTEGER`, `ContactAddr`: `VARCHAR`, `LastPing`: `DATETIME`}
	_            = bytes.MinRead
)
